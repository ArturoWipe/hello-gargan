...
* it can be heavy, but always well decomposed (eg. the [Diffgram project](https://github.com/diffgram/diffgram/tree/master/frontend/src/components), or simply by adhering to nested folders)
...
* basically, the Store implemented in this project is not so far from what we can see with [Vuex](https://vuex.vuejs.org/), ie:
...
* first of all recent JavaScript frontend libraries offers a simple yet comprehensive creation/structural/behavioral paradigm. ReactJS (Hooks API) opts for a fully JavaScript-ly functional composition paradigm. VueJS adheres to a [Single-File-Component](https://vuejs.org/v2/guide/single-file-components.html) paradigm. The former more difficult to understand for newcomers, but the latter less flexible. It is not a surprise here, but Reactix + Toestand in a PureScript context *is* the most reliable paradigm
...
There is a distinction to be noted here, mainly due to PureScript paradigms. As a fully FP language, it is a requirement to be explicit with null coalescing values. But if we look closer, eg. by taking the "purescript-react-basic" [library](https://github.com/lumihq/purescript-react-basic/blob/v0.7.1/src/React/Basic/DOM.purs#L4-L6), we can see an inclination opposite to this requirement. The question is simple as that: in what measures can we write PureScript component specs by knowing this variadic arity in its attributes/props?
...
With modern framework, every frontend applications have know the requirement of a Boostrap set of components. Indeed, as we can simply import a frontend CSS + JS + HTML library (such as [Material Design](https://material.io/design) or [Bootstrap](https://getbootstrap.com/)), most of their vanilla components will inherit of: a possible logic (that it is best to implement within the paradigm of the language/frontend library/framework) ; or a set of constants.


At this moment in PureScript, only few examples remain (we can think of ["purescript-react-material-ui"](https://github.com/nwolverson/purescript-react-material-ui)). This project started a set of components for Bootstrap v4 (and ["react-bootstrap"](https://react-bootstrap.github.io/) can be an inspiration to look for). The project will also contains basic components independent from Bootstrap, but registered as "bootstrap" (ie. "bootstrap" semantic as in reusable set of components).
...
- [ ] `<blank-placeholder>` component: this is a truly needed addition, please check the "Placeholder" part of the [Component Cookbook](wiki/a_cookbook_to_component_style_patterns.md)
...
3. idem, but with a UI/UX point of view: is the modal content a new page/view? or is it a fully wrapped-isolated page/view (such as an iframe)?
4. unwanted added complexity, this is a common smelly things happening with modals: when we use a modal in a page, it is anticipated that actions made in within it will have an action on the page itself. How behavior should be taken into account? when the modal is still there (ie. by making some UI feedbacks behind the modal)? or waiting for the modal to be closed?
